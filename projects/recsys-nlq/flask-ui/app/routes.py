...
def search():
    ...
    try:
        df = load_products_from_gcs()
        response = get_recommendations('nlq2item', query=query, limit=100)
        search_similar = response["results"]
        knn_params = response.get("knn_params", {})
        knn_params_str = json.dumps(knn_params, indent=2)
        products = get_products_by_skus(df, search_similar).to_dict('records')
        
        return render_template('search_results.html',
                             products=products,
                             query=query,
                             params=knn_params_str,
                             hierarchy=get_nested_category_hierarchy(df)
        )
