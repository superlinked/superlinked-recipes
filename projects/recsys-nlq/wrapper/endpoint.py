...
def call_sl_url(path_name, data=None, include_metadata=False):
...
async def get_item_by_nli(query, limit=10):
    ...
    query_data['limit'] = int(limit)
    try:
        _, results = call_sl_url("nlq2item", data=query_data, include_metadata=True)
        knn_params = {}
        for key in results["metadata"]["search_params"].keys():
            if not key.endswith("__"): ## remove internal parmetes of superlinked from UI
                knn_params[key] = results["metadata"]["search_params"][key]
                
        app_state.logger.info(f"\n\n NLQ results: {knn_params} \n\n")
        results = {
            "results": clean_metadata_from_results(results['entries']),
